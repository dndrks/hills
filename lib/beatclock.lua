...
function BeatClock.new(name)
	...
	i.metro = metro.init()
	i.metro.count = -1
	i.metro.event = function()
		i:tick()
	end
	i:bpm_change(110)

	i.on_step = function(e)
		print("BeatClock executing step")
	end
	i.on_start = function(e) end
	i.on_stop = function(e) end
	i.on_select_internal = function(e)
		print("BeatClock using internal clock")
	end
	i.on_select_external = function(e)
		print("BeatClock using external clock")
	end
...
function BeatClock:add_clock_params()
	...
	params:set_action("clock", function(x)
		self:clock_source_change(x)
	end)
	...
	params:set_action("clock_out", function(x)
		if x == 1 then
			self.send = false
		else
			self.send = true
...
--- process midi
-- this should be sent MIDI data from an incoming connection
